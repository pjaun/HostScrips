...
CONFIG=${1:-/root/backupmysql/backupmysql.conf}
...
if  [ $DUMPALL = "y" ]; then
        ...
        DBS=`docker exec $HOST mysql -h $HOST --user=$USER --password=$PASS -Bse "show databases;" | grep -Ev "^(${EXCLUDED_DBS})$"`
        echo "done!"
fi
...
for database in $DBS; do
        ...
        docker exec $HOST mysqldump -h $HOST --user=$USER --password=$PASS --routines=true $database > \
